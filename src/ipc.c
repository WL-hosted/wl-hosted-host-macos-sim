...
static uint16_t read16(const uint8_t *p) { return (uint16_t)p[0] | (uint16_t)((uint16_t)p[1] << 8u); }
...
{
    p[0] = (uint8_t)v; p[1] = (uint8_t)(v >> 8u); p[2] = (uint8_t)(v >> 16u);
    p[3] = (uint8_t)(v >> 24u);
}
...
int sim_ipc_open(sim_ipc_t *ipc, const char *endpoint)
{
    ...
    char *end;
    long inherited;
    if (ipc == NULL || endpoint == NULL) return -1;
    ...
    if (strncmp(endpoint, "connect:", 8u) == 0) {
        ...
        if (errno != 0 || *end != '\0' || inherited < 0 || inherited > INT32_MAX) return -1;
        ipc->fd = (int)inherited;
    }
    if (ipc->fd < 0) return -1;
    ...
    if (write_all(ipc->fd, local, sizeof(local)) != 0 || read_all(ipc->fd, peer, sizeof(peer)) != 0 ||
        memcmp(peer, hello_magic, sizeof(hello_magic)) != 0 || read16(peer + 8u) != 1u ||
        (peer[10] != SIM_ROLE_MANAGER && peer[10] != SIM_ROLE_COPROC) ||
        (peer[11] & (uint8_t)~SIM_IPC_SIDEBAND_FLAG) != 0u || read32(peer + 12u) < 4u) {
        sim_ipc_close(ipc);
        return -1;
    }
    ipc->peer_role = (enum sim_role)peer[10];
    ipc->max_record_size = read32(peer + 12u) < SIM_IPC_MAX_RECORD_SIZE
                               ? read32(peer + 12u) : SIM_IPC_MAX_RECORD_SIZE;
...
int sim_ipc_write(sim_ipc_t *ipc, uint8_t kind, const uint8_t *payload, size_t payload_size)
{
    ...
    if (ipc == NULL || ipc->fd < 0 || kind < SIM_RECORD_WIRE_FRAME ||
        kind > SIM_RECORD_FAULT_RESPONSE || (payload_size != 0u && payload == NULL) ||
        body_size > ipc->max_record_size) return -1;
    record = malloc(4u + body_size);
    if (record == NULL) return -1;
    write32(record, (uint32_t)body_size);
    record[4] = kind; record[5] = 0u; record[6] = 0u; record[7] = 0u;
    if (payload_size != 0u) memcpy(record + 8u, payload, payload_size);
...
int sim_ipc_read(sim_ipc_t *ipc, uint8_t *kind, uint8_t **payload, size_t *payload_size)
{
    ...
    uint8_t fixed[4];
    uint32_t body_size;
    if (ipc == NULL || kind == NULL || payload == NULL || payload_size == NULL ||
        read_all(ipc->fd, length_bytes, sizeof(length_bytes)) != 0) return -1;
    body_size = read32(length_bytes);
    if (body_size < 4u || body_size > ipc->max_record_size ||
        read_all(ipc->fd, fixed, sizeof(fixed)) != 0 || fixed[0] < SIM_RECORD_WIRE_FRAME ||
        fixed[0] > SIM_RECORD_FAULT_RESPONSE || fixed[1] != 0u || fixed[2] != 0u ||
        fixed[3] != 0u) return -1;
    *payload_size = body_size - 4u;
    *payload = *payload_size == 0u ? NULL : malloc(*payload_size);
    if (*payload_size != 0u && (*payload == NULL || read_all(ipc->fd, *payload, *payload_size) != 0)) {
