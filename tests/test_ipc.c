...
static int read_all(int fd, uint8_t *data, size_t size)
{
    while (size != 0u) {
        ...
        if (got <= 0) return -1;
        data += (size_t)got; size -= (size_t)got;
    }
    return 0;
}
static int write_all(int fd, const uint8_t *data, size_t size)
{
    while (size != 0u) {
        ...
        if (put <= 0) return -1;
        data += (size_t)put; size -= (size_t)put;
    }
    return 0;
}
...
static void *peer_main(void *context)
{
    ...
    uint8_t prefix[4];
    uint8_t body[64];
    static const uint8_t response[16] = {'W','L','H','S','I','M',0,0,1,0,3,1,27,0,1,0};
    ...
    assert(body[0] == SIM_RECORD_WIRE_FRAME && memcmp(body + 4u, "abc", 3u) == 0);
    prefix[0] = 7u; prefix[1] = prefix[2] = prefix[3] = 0u;
    body[0] = SIM_RECORD_WIRE_FRAME; body[1] = body[2] = body[3] = 0u;
...
int main(void)
{
    ...
    assert(sim_ipc_write(&ipc, SIM_RECORD_WIRE_FRAME, (const uint8_t *)"abc", 3u) == 0);
    assert(sim_ipc_read(&ipc, &kind, &payload, &payload_size) == 0);
    assert(kind == SIM_RECORD_WIRE_FRAME && payload_size == 3u && memcmp(payload, "xyz", 3u) == 0);
    free(payload);
    sim_ipc_close(&ipc);
